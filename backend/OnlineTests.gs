...
// ── OnlineTests sheet columns (22 total) ──────────────────────
...
function ensureOTHeaders(sh) {
  ...
  const h=[
    ...
    'Allow Retake','Shuffle Questions','Instructions','Scheduled Activate At'
  ];
  sh.getRange(1,1,1,h.length).setValues([h]).setFontWeight('bold').setBackground(NAVY).setFontColor(GOLD).setFontFamily('Arial');
  sh.setFrozenRows(1);
}
...
function otGetQuestionBank(ss, p) {
  ...
  var questions=rows.filter(function(r){return r[0]&&r[3];})
    ...
    .map(function(r){return{id:r[0],course:r[1],topic:r[2],question:r[3],opt1:r[4],opt2:r[5],opt3:r[6],opt4:r[7],type:r[9]||'MCQ'};});
